...
// lib/api/pixlate.ts)가 완전히 같은 경로 `/jobs/{jobId}`를 공유한다.
...
// `x-pixlate-api-proxy`를 붙이고, next.config.ts의 `/jobs/:path*` rewrite는
...
const PROXY_HEADER = 'x-pixlate-api-proxy';
...
test('mock enabled — /jobs/{jobId} document navigation은 헤더 없이 페이지에 도달하고, API fetch만 프록시 식별 헤더를 단다', async ({
  ...
  // 3. document request에는 x-pixlate-api-proxy가 없음
